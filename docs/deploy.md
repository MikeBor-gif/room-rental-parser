...
|---|---|---|
| `bot.yml` | слушает Telegram ~4 мин за прогон (ответы через секунды) | каждые **3 минуты** |
...
**Задание 1 — bot (каждые 3 минуты) — создать НОВОЕ:**

- **URL:** `https://api.github.com/repos/MikeBor-gif/room-rental-parser/actions/workflows/bot.yml/dispatches`
- **Schedule:** every 3 minutes
  (прогон слушает Telegram ~4 минуты; следующий дёрг перехватывает эстафету —
  ЧАЩЕ ставить нельзя: дёрг каждую минуту убивал бы слушателя посреди работы)
- **Request method:** POST

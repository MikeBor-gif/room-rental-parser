...
HELP = (
    ...
    "/start — главное меню\n"
    "/add — добавить фильтр\n"
    "/filters — мои фильтры (изменить/удалить)\n"
    "/premium — премиум-подписка\n"
    "/pause — приостановить рассылку\n"
    "/resume — возобновить рассылку\n"
    "/help — эта справка\n\n"
    "⏳ <i>Ответы приходят с задержкой до пары минут — это нормально.</i>"
)
...
def fmt_filter_saved(property_type: str, city_code: str, max_price: float | None) -> str:
    ...
    return (
        ...
        "Мои фильтры: /filters · Ещё фильтр: /add"
    )
...
def fmt_filter_limit_free(price_byn: float) -> str:
    return (
        ...
        "уведомления: /premium\n\n"
        "Либо удалите текущий фильтр (/filters) и создайте новый."
    )
...
def fmt_filter_limit_premium(max_filters: int) -> str:
    return (
        ...
        "Удалите ненужный (/filters), чтобы добавить новый."
    )
